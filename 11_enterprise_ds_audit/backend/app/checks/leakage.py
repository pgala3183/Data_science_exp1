...
def check(_root: Path, files: list[SourceFile]) -> list[Finding]:
    ...
    for sf in files:
        ...
        # Skip unit-test files for this heuristic — they often mention banned columns by name.
        rel_lower = sf.relative.lower()
        in_tests = rel_lower.startswith("tests/") or "/tests/" in rel_lower or rel_lower.startswith("test_")
        if not in_tests:
